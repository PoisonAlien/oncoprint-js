...
export interface CohortInfo {
  // Option 1: Provide total count only
  totalSamples?: number;
  
  // Option 2: Provide complete sample list
  samples?: string[];
...
export interface SampleGroup {
  value: string;           // The annotation value (e.g., "BRCA", "LUAD")
  samples: string[];       // Samples in this group
  count: number;          // Number of samples
  startIndex: number;     // Starting position in the combined view
  endIndex: number;       // Ending position in the combined view
}
...
export interface ProcessedData {
  ...
  percentageCalculationBase: number; // Denominator for percentage calculations
  cohortInfo?: {
    totalSamples: number;
    providedSamples?: string[];
    missingSamples?: string[];
  };
  sampleGroups?: SampleGroup[]; // Groups for split visualization
}
...
export interface MetadataTrackConfig {
  ...
  showLabels?: boolean;
  tooltips?: boolean;
  customRenderer?: (value: string | number, sample: string) => { color: string; tooltip?: string };
}
...
export interface OncoprintConfig {
  ...
  // Metadata tracks (legacy support)
  ...
  // Split visualization
  splitBy?: {
    field: string;              // Metadata field to split by
    gapSize?: number;           // Gap size between groups (default: 20px)
    showGroupHeaders?: boolean; // Show group labels (default: true)
    showGroupCounts?: boolean;  // Show sample counts (default: true)
  };
