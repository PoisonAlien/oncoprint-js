...
class VariantColorManager {
    constructor(predefinedColors = DEFAULT_VARIANT_COLORS) {
        ...
        this.colorPalette = [
            ...d3__namespace.schemeCategory10,
            ...d3__namespace.schemeSet3
        ];
...
class OncoprintRenderer extends EventEmitter {
    ...
    createFullSizeExportSVG() {
        ...
        const tempSVG = d3__namespace.select(tempContainer)
            .append('svg')
    ...
    createSVG() {
        // Remove existing SVG
        d3__namespace.select(this.container).selectAll('svg').remove();
    ...
    createColorScale(field, fieldType, trackConfig) {
        if (fieldType === 'numerical') {
            ...
            if (!extent && allValues.length > 0) {
                extent = d3__namespace.extent(allValues);
            ...
            switch (trackConfig.colorScale) {
                case 'viridis':
                    ...
                    break;
                case 'plasma':
                    interpolator = d3__namespace.interpolatePlasma;
                    break;
                case 'reds':
                    interpolator = d3__namespace.interpolateReds;
                    break;
                case 'greens':
                    interpolator = d3__namespace.interpolateGreens;
                    break;
                default:
                    interpolator = d3__namespace.interpolateBlues;
                    break;
            }
            return d3__namespace.scaleSequential(interpolator).domain(extent);
        }
        else {
            ...
            if (trackConfig.colors) {
                if (Array.isArray(trackConfig.colors)) {
                    return d3__namespace.scaleOrdinal(trackConfig.colors).domain(allValues);
                }
                    ...
                    switch (trackConfig.colors) {
                        case 'category10':
                            ...
                            break;
                        case 'category20':
                            colorScheme = d3__namespace.schemeCategory10.concat(d3__namespace.schemeCategory10);
                            break;
                        case 'pastel1':
                            colorScheme = d3__namespace.schemePastel1;
                            break;
                        case 'set3':
                            colorScheme = d3__namespace.schemeSet3;
                            break;
                        default:
                            colorScheme = d3__namespace.schemeCategory10;
                            break;
                    }
                    return d3__namespace.scaleOrdinal(colorScheme).domain(allValues);
                }
                else {
            ...
            const defaultScheme = allValues.length <= 10 ? d3__namespace.schemeCategory10 :
                allValues.length <= 20 ? d3__namespace.schemeCategory10.concat(d3__namespace.schemePastel1) :
                    d3__namespace.schemeCategory10.concat(d3__namespace.schemePastel1).concat(d3__namespace.schemeSet3);
            return d3__namespace.scaleOrdinal(defaultScheme).domain(allValues);
        }
    }
    renderLegend() {
    ...
    renderNumericalMetadataLegend(parentGroup, trackConfig, startX, y) {
        ...
        if (!values.length)
            return;
        const extent = trackConfig.domain || d3__namespace.extent(values);
...
const Oncoprint = react.forwardRef(({ mafData, metadataData, mafFile, metadataFile, config = {}, width, height, onGeneClick, onSampleClick, onCellClick, onDataLoaded, onError, onRenderComplete, className, style }, ref) => {
    const containerRef = react.useRef(null);
    const visualizerRef = react.useRef(null);
...
function useOncoprint({ container, config = {}, autoRender = true } = {}) {
    ...
    // Data loading methods
    const loadMafFile = react.useCallback(async (file) => {
        if (!visualizerRef.current) {
            throw new Error('Visualizer not initialized');
        }
        setIsLoading(true);
    ...
    const loadMafData = react.useCallback(async (mafData) => {
        if (!visualizerRef.current) {
            throw new Error('Visualizer not initialized');
        }
        setIsLoading(true);
    ...
    const loadMetadataFile = react.useCallback(async (file) => {
        if (!visualizerRef.current) {
            throw new Error('Visualizer not initialized');
        }
        setIsLoading(true);
    ...
    const loadMetadataData = react.useCallback(async (metadataData) => {
        if (!visualizerRef.current) {
            throw new Error('Visualizer not initialized');
        }
        setIsLoading(true);
    ...
    const resize = react.useCallback((width, height) => {
        visualizerRef.current?.resize(width, height);
    }, []);
    // Export methods
    ...
    // Selection methods
    const setGeneSelection = react.useCallback((genes) => {
        setSelectedGenes(genes);
        visualizerRef.current?.setGeneSelection(genes);
    }, []);
    const setSampleSelection = react.useCallback((samples) => {
        setSelectedSamples(samples);
        visualizerRef.current?.setSampleSelection(samples);
    }, []);
    // Analysis methods
    const sortGenesByFrequency = react.useCallback((descending = true) => {
        visualizerRef.current?.sortGenesByFrequency(descending);
    }, []);
    const sortSamplesByMutationLoad = react.useCallback((descending = true) => {
        visualizerRef.current?.sortSamplesByMutationLoad(descending);
    }, []);
    const sortSamplesByMetadata = react.useCallback((field, ascending = true) => {
        visualizerRef.current?.sortSamplesByMetadata(field, ascending);
    }, []);
    const filterByMutationFrequency = react.useCallback((minFreq, maxFreq = 1) => {
        visualizerRef.current?.filterByMutationFrequency(minFreq, maxFreq);
    }, []);
    return {
        // State
...
exports.DEFAULT_VARIANT_COLORS = DEFAULT_VARIANT_COLORS;
exports.DataProcessor = DataProcessor;
exports.EventEmitter = EventEmitter;
exports.MafParser = MafParser;
exports.MetadataParser = MetadataParser;
exports.Oncoprint = Oncoprint;
exports.OncoprintRenderer = OncoprintRenderer;
exports.OncoprintSimple = OncoprintSimple;
exports.OncoprintVisualizer = OncoprintVisualizer;
exports.VariantColorManager = VariantColorManager;
exports.useOncoprint = useOncoprint;
